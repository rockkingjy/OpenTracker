...
void OptimizeScores::compute_scores()
{
	...
	for (int ite = 0; ite < iterations_; ++ite)
	{
		...
		for (unsigned int i = 0; i < scores_fs_.size(); i++)
		{
			ky_exp_ky.push_back(complexMultiplication(real2complex(kyMat), exp_iky[i]));
			kx_exp_kx.push_back(complexMultiplication(real2complex(kxMat), exp_ikx[i]));
			...
			H_yy.push_back(real(-1 * complexMultiplication(real2complex(ky2Mat), exp_iky[i]) * resp_x[i] + ival[i]));
			H_xx.push_back(real(-1 * y_resp[i] * complexMultiplication(real2complex(kx2Mat), exp_ikx[i]) + ival[i]));
