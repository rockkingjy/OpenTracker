...
struct EcoParameters
{
	...
	float learning_rate = 0.01;//0.009;			// Learning rate
	...
	float projection_reg = 5e-7;//1e-7;	  // Regularization paremeter of the projection matrix (lambda)
	...
	int init_CG_iter = 10 * 20;//15;		 // The total number of Conjugate Gradient iterations used in the first frame
	...
	int CG_forgetting_rate = 60;//50;	 	 	   // Forgetting rate of the last conjugate direction
	float precond_data_param = 0.75; //0.3;	 	   // Weight of the data term in the preconditioner
	float precond_reg_param = 0.2;//0.25;   	   // Weight of the regularization term in the preconditioner
	...
	double reg_window_edge = 4e-3;//10e-3;		 // The impact of the spatial regularization
	size_t reg_window_power = 2;		 // The degree of the polynomial to use(e.g. 2 is a quadratic window)
	float reg_sparsity_threshold = 0.15;//0.05; // A relative threshold of which DFT coefficients that should be set to zero
