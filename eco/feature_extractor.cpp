...
ECO_FEATS feature_extractor::extractor(cv::Mat image, cv::Point2f pos, vector<float> scales, 
						const eco_params& params,const cv::Mat& yml_mean, 
						bool useDeepFeature, const boost::shared_ptr< Net<float> >& net)
{
	...
	for (int i = 0; i < num_features; ++i)
	{
		...
		{
			cv::Size2f img_sample_sz = (i == 0) && useDeepFeature ? cnn_features.img_sample_sz : hog_features.img_sample_sz;
			cv::Size2f img_input_sz =  (i == 0) && useDeepFeature ? cnn_features.img_input_sz : hog_features.img_input_sz;
...
{
	//cv::Mat mean = meanMatFromYML("mean.yml");
